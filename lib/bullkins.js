...
async function processBullkins(job, done) {
  ...
  try {
    ...
    // Bullkins Job 登録 WebAPI 呼び出し関数
    job.$register = async (yaml, type = 'jobs') => {
      return await axios.post(`http://localhost:8000/api/bullkins/${type}`, jsyaml.dump(yaml), {
 ...
 *          Bullkins WebAPI を呼び出して指定の Yaml object をジョブとして登録する（option.attempts, option.backoff による再試行が上手く行かない場合用）
